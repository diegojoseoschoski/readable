...
import { ROOT_URL, AUTH_HEADERS, guid } from "../constants";
...
export function getPostCommentsCount(postId, callback) {
  return dispatch => {
    axios.get(`${ROOT_URL}/posts/${postId}/comments`).then(res => {
      ...
      callback(data);
      dispatch({ type: GET_POST_COMMENTS_COUNT, payload: data });
    });
  };
}
...
export function fetchCommentPost(id) {
  return dispatch => {
    axios
      ...
      .then(res => dispatch({ type: GET_COMMENT_POST, payload: res.data }));
  };
}
